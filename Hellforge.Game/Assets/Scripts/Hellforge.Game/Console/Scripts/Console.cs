...
namespace Hellforge.Game
{
    ...
    public static class Console
    {
        ...
        {
            return RefreshCommands(new Assembly[] { Assembly.GetAssembly(typeof(Console)) });
        }
        ...
        public static int RefreshCommands(Assembly[] assemblies)
        {
            ...
            foreach(var command in ConsoleHelper.FindMethodAttributes<ConCommand>(assemblies))
            {
            ...
            foreach (var convar in ConsoleHelper.FindPropertyAttributes<ConVar>(assemblies))
            {
        ...
        public static List<ConObject> GetAllConObjectsThatMatch(string input)
        {
            ...
            foreach (var command in Commands)
            {
                ...
                if(cleanName.StartsWith(input) || ConsoleHelper.LevenshteinDistance(cleanName, input) < 2 && cleanName != input)
                {
                    objects.Add(command);
                }
            }
        ...
        public static void ExecuteString(string input)
        {
            ...
            if (command != null) //if the command exists
            {
                ...
                {
                    ExecuteCommand((ConCommand)command, ConsoleHelper.CastParameters(cleanParts));
                }
                else //if no command, then convar
                {
                    ExecuteConvar((ConVar)command, ConsoleHelper.CastParameters(cleanParts));
                }
            }
        }
    }

...
namespace Hellforge.Game.UI
{
    public class ConsolePanelController : MonoBehaviour
    {
        ...
        {
            var output = "";
            foreach(var line in StringBuffer)
            {
                output += line + "\n";
            }
        ...
        public void CommandEntered(string input)
        {
            ...
            var command = Console.GetConObjectByName(commandName); //get either the convar or concommand typed
            if (command != null) //if the command exists
            {
                string[] cleanParts = ConsoleHelper.CleanArgumentsArray(parts, command.GetParametersLength());

                Console.print($"<color=#ffffffaa>{input}</color>");
                ...
                if (command is ConCommand)
                {
                    ...
                    {
                        object commandResult = Console.ExecuteCommand((ConCommand)command, ConsoleHelper.CastParameters(cleanParts));
                    ...
                    {
                        Console.ExecuteCommand((ConCommand)command, ConsoleHelper.CastParameters(cleanParts));
                    }
                }
                else //if no command, then convar
                {
                    object result = Console.ExecuteConvar((ConVar)command, ConsoleHelper.CastParameters(cleanParts));
                    if (result != null)
                    {
                        if (command.GetDescription() == "")
                        {
                            Console.print($"<color=#ffffffaa>{commandName} = {result}</color>");
                        }
                        else
                        {
                            Console.print($"<color=#ffffffaa>{commandName} = {result}\n\"{command.GetDescription()}\"</color>");
                        }
                    }
                }
            }
            ...
            {
                Console.print($"Command '{commandName}' does not exist!");
            }
        }
